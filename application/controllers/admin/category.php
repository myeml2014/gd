...
class Category extends CI_Controller {
	...
	public function index()
	{
		...
		$footerData = array();
		$data = array();
		$modulePathUrl = BASE_URL.APPPATH.'views/admin/category/';
		...
		$headerData['module_js'] = $modulePathUrl.'category.js';
		$this->load->view('admin/header/header',$headerData);
		$this->load->view('admin/category/category',$data);
		$footerData['module_path'] = ADMIN_VIEW_PATH."category/";
	...
	function load($start=0,$param='')
	{
		...
		if($param != '')
		$whArr = explode(",",$param);
		$data = $this->category_model->get_all($start,$whArr);
		$objResponse->script('renderJson('.json_encode($data).','.$this->category_model->get_all_count($whArr).')');
		return $objResponse;
	}
	function setForm($id)
	{
		...
		$objResponse->assign("txtCatNm","value",$data->cat_name);
		$objResponse->assign("txtCatDesc","value",$data->cat_desc);
		$objResponse->assign("selIsMain","value",$data->is_main_menu);
	...
	{
		$objResponse=new xajaxResponse();
		if($this->category_model->checkIsExists($arg['txtCatNm'],$id))
		{
		    return $objResponse->alert($this->lang->line('msg_exist'));
		}
		$arrVal = array();
